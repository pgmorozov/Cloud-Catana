...
The project uses the Microsoft Identity Platform (a.k.a Azure Active Directory) as its identity provider to authenticate clients. This feature requires registered Azure AD applications (Client and Server) which allows users to connect to the Azure Function app using OAuth authentication.
